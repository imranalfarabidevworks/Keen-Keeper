...
function UtilityButton({ label, icon, danger = false }) {
  return (
    <button
      type="button"
      className={[
        "inline-flex w-full items-center justify-center gap-2 rounded border border-slate-200 px-3 py-2 text-md font-medium transition",
...
export default function FriendDetailsPage() {
  ...
  if (!friend) {
    return (
      <section className="rounded-lg border border-slate-200 bg-white px-6 py-16 text-center shadow-md">
        ...
        <article className="rounded-md border border-slate-200 bg-white px-6 py-6 text-center shadow-sm">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-center gap-1.5">
            {friend.tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-lime-100 px-2 py-0.5 text-[10px] font-semibold uppercase text-green-700"
              >
                {tag}
              </span>
            ))}
          </div>
          <p className="mt-4 text-lg italic text-slate-600">"{friend.bio}"</p>
          <p className="mt-2 text-md text-slate-400">{friend.email}</p>
        </article>
      ...
      <div className="space-y-4">
        ...
        <article className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              type="button"
              className="rounded border border-slate-200 px-3 py-1 text-md text-slate-500 hover:bg-slate-50"
            >
              Edit
            </button>
          </div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-3">
            {QUICK_ACTIONS.map((action) => (
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-md border border-slate-200 px-4 py-3 text-md text-slate-700 transition hover:bg-slate-50"
              >
